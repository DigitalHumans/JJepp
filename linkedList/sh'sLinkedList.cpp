...
void append(Node *list, int value);              //링크드리스트의 마지막에 요소를 추가
void change(Node *list, int index, int value);   //링크드리스트의 특정 요소의 값 수정
void insert(Node **list, int index, int value);   //링크드리스트의 특정 위치에 요소 추가
void remove(Node **list, int index);              //링크드리스트의 특정 요소 삭제
int  getValue(Node *list, int index);            //링크드리스트의 특정 요소 값 리턴
void printList(Node *list);                      //링크드리스트 순서대로 출력

int main()
{
    Node *list = new Node(1);           //리스트 생성
    printList(list);                    //{ 1 }
...
void insert(Node **list, int index, int value)   //링크드리스트의 특정 위치에 요소 추가
{
    ...
    for(int i=0; i<index; i++)
    {
        ...
        {
            Node *pre_tmp = p;              //노드 연결을 위해 추가할 인덱스 전 노드 주소 저장
            p = p->next_node;               //추가할 노드 다음 노드를 가리킴
            Node *tmp = new Node(value);    //추가할 노드 생성
...
void remove(Node **list, int index)              //링크드리스트의 특정 요소 삭제
{
    ...
    {
        Node *x_tmp = *list;               //delete를 위해 주소 저장
    ...
    for(int i=0; i<index; i++)
    {
        if(i == index-1)           //인덱스 바로 앞 노드를 찾으면
        {
            Node *pre_tmp = p;              //노드 연결을 위해 삭제할 인덱스 전 노드 주소 저장
            p = p->next_node;               //삭제할 노드를 가리킴
            Node *x_tmp = p;                //delete를 위해 주소 저장
